...
class View {
    ...
    fun createVisualizaion(dataset: String, graphType: String) {
        ...
        when (graphType) {
            ...
            "Scatter" -> {
                createScatterGraph(dataset)
            }
        }
    }

    private fun createScatterGraph(dataset: String) {
    ...
    fun createBarSEMGraph(dataset: String) {
        ...
        canv.graphicsContext2D.apply {
            ...
            val ssd = sqrt(tmp_sum / (n - 1))
            val sem = sd / sqrt(n.toDouble())
            val confidenceWidth = 1.96 * ssd / sqrt(n.toDouble())
            setLineDashes(5.0)
            ...
            stroke = Color.RED
            val confidenceU = canvHeight - (mean + confidenceWidth - mins) * canvHeight / ydiff + 5
            val confidenceD = canvHeight - (mean - confidenceWidth - mins) * canvHeight / ydiff + 5
            strokeLine(10.0, confidenceU, canvWidth + 30, confidenceU)
            strokeLine(10.0, confidenceD, canvWidth + 30, confidenceD)
            ...
            fillText("SEM: $sem", 30.0, 50.0)
            fillText("Confidence interval: (${(100 * (mean - confidenceWidth)).toInt() / 100.0}, " +
                    "${(100 * (mean + confidenceWidth)).toInt() / 100.0}) ", 30.0, 70.0)
        }
    }
